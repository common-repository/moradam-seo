(function ( $) {
    ...
    window.onbeforeunload = function () {
        ...
        localStorage.removeItem('moradam_rankings_top_filter');
        localStorage.removeItem('moradam_kw_on_top_rankings_filter_keyword');
        localStorage.removeItem('moradam_kw_on_top_rankings_filter_url');
    };
    ...
    $(document).ready(function() {
        ...
        $('#keywords-daterange-inner').on('apply.daterangepicker', function(ev, picker) {
            if (!localStorage.getItem('moradam_seo_selected_se')) { return; }
            localStorage.removeItem('moradam_kw_offset');
        ...
        $('#rankings-filter-kw-apply-btn').click(function() {
            if ($('#rankings-keyword-filter-input').val().length > 0) {
                ...
                localStorage.setItem('moradam_kw_on_top_rankings_filter_keyword', inputKeyword);
                localStorage.removeItem('moradam_kw_offset');
                remove_all_in_tbody();
                get_rankings_data();
                $("#rankings-keyword-filter-block").hide();
        ...
        $('#rankings-filter-url-apply-btn').click(function() {
            if ($('#rankings-url-filter-input').val().length > 0) {
                ...
                localStorage.setItem('moradam_kw_on_top_rankings_filter_url', inputKeyword);
                localStorage.removeItem('moradam_kw_offset');
                remove_all_in_tbody();
                setTimeout(function() {
                    get_rankings_data();
        ...
        $('#keyword-filter-select-top-count').change(function () {
            ...
            localStorage.removeItem('moradam_kw_offset');
            localStorage.setItem('moradam_rankings_top_filter', selected);
            remove_all_in_tbody();
            scrollCounter = 0;
            setTimeout(function () {
                ...
                tooltip: {
                    ...
                    callbacks: {
                        ...
                        title: function(tooltipItem, data) {
                            return 'Keywords in TOP on ' + moment(tooltipItem[0].label, 'YYYY-MM-DD').format("MMM Do, YYYY");
                        }
                    }
                }
            },
    ...
    function get_rankings_data(top_filter = '') {
        ...
        if (localStorage.getItem('moradam_kw_on_top_rankings_filter_keyword')) {
            request_data.kw_filter_type = 'match';
            request_data.kw_filter = localStorage.getItem('moradam_kw_on_top_rankings_filter_keyword');
        }
        if (localStorage.getItem('moradam_kw_on_top_rankings_filter_url')) {
            request_data.page_filter_type = 'match';
            request_data.page_filter = localStorage.getItem('moradam_kw_on_top_rankings_filter_url');
        }
        if (top_filter) {
            request_data.top_filter = top_filter;
        } else if (localStorage.getItem('moradam_rankings_top_filter')) {
            request_data.top_filter = localStorage.getItem(('moradam_rankings_top_filter'));
        } else {
            ...
            success: function(resp) {
                setTimeout(function() {
                    if (resp.result.total > 0) {
                        ...
                        insert_moradam_kw_into_table(resp.result.rows);
                    } else {
    ...
    function insert_moradam_kw_into_table(keywords, lastId) {
        $('.no-data-found').remove();
        ...
        for (let i = 0; i <= keywords.length-1; i++) {
            ...
            let tdGoogle = '<td class="center"><a href="https://www.google.com/search?q=' + keywords[i].kw + '&hl=all&start=0&gws_rd=cr" target="_blank"><img class="google_icon" src="/wp-content/plugins/moradam-seo/admin/img/google.svg"></a></td>';
    ...
    $(window).scroll(function () {
        ...
        if($(window).scrollTop() == $(document).height() - $(window).height()) {
            ...
            if (allowScrollLoad == true) {
                ...
                if (localStorage.getItem('moradam_kw_offset')) {
                    request_data.offset = localStorage.getItem('moradam_kw_offset');
                } else {
                    localStorage.setItem('moradam_kw_offset', defaultOffset);
                    request_data.offset = localStorage.getItem('moradam_kw_offset');
                }
                if (localStorage.getItem('moradam_kw_on_top_rankings_filter_keyword')) {
                    request_data.kw_filter_type = 'match';
                ...
                if (localStorage.getItem('moradam_rankings_top_filter')) {
                    request_data.top_filter = localStorage.getItem('moradam_rankings_top_filter');
                } else {
                    request_data.top_filter = 100;
                }
                    ...
                    $.ajax({
                        ...
                        success: function(resp) {
                            if (resp.result != null && resp.result.total > 0) {
                                allowScrollLoad = true;
                                insert_moradam_kw_into_table(resp.result.rows, lastId);
                                if (resp.result.rows.length == size) {
                                    let offset = localStorage.getItem('moradam_kw_offset');
                                    let newOffset = parseInt(offset) + parseInt(size);
                                    localStorage.setItem('moradam_kw_offset', newOffset );
                                }
                            }
                            $('.preloader-keywords-table').css('display', 'none');
                        },
