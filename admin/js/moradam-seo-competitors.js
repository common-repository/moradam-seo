(function ( $) {
    ...
    window.onbeforeunload = function () {
        localStorage.removeItem('moradam_competitors_offset');
        localStorage.removeItem('moradam_seo_selected_se');
        localStorage.removeItem('moradam_competitors_top_filter');
        localStorage.removeItem('moradam_competitors_dynamics_top_filter');
        localStorage.removeItem('moradam_competitors_selected_domains');
        localStorage.removeItem('moradam_competitors_filter_domain');
    };
    ...
    $(document).ready(function() {
        ...
        $('#competitors-filter-select-top-count').change(function () {
            ...
            localStorage.removeItem('moradam_competitors_offset');
            localStorage.setItem('moradam_competitors_top_filter', selected);
            remove_all_in_tbody();
            scrollCounter = 0;
            setTimeout(function() {
                get_competitors_in_google_top();
        ...
        $('#competitors-dynamics-filter-select-top-count').change(function () {
            let selected = $(this).find(':selected').val();
            localStorage.setItem('moradam_competitors_dynamics_top_filter', selected);
            get_charts_data();
        });
        ...
        $('#competitors-filter-domain-apply-btn').click(function() {
            if ($('#competitors-domain-filter-input').val().length > 0) {
                ...
                localStorage.setItem('moradam_competitors_filter_domain', inputKeyword);
                localStorage.removeItem('moradam_kw_offset');
                remove_all_in_tbody();
                get_competitors_in_google_top();
                $("#competitors-domain-filter-block").hide();
            }
        ...
        $(document).on('change', '.checkbox-tbl', function() {
            ...
            if ($(this).prop('checked') == true) {
                let selected_domains = JSON.parse(localStorage.getItem('moradam_competitors_selected_domains'));
                let hash;
                let params = {
                ...
                $.ajax({
                    ...
                    success: function(resp) {
                        if (resp.result) {
                            ...
                            localStorage.removeItem('moradam_competitors_selected_domains');
                            localStorage.setItem('moradam_competitors_selected_domains', selected_domains);
                        }
                        get_charts_data();
                    },
                });
    ...
    function get_competitors_rank() {
        ...
        let selected_se = localStorage.getItem('moradam_seo_selected_se');
        if (selected_se) {
        ...
        $.ajax({
            ...
            success: function(resp) {
                ...
                $("#moradam-rank-position-number").text(rank);
                $("#moradam-rank-keywords-count").text(kwCount);
            },
        });
    }

    function clear_competitors_rank() {
        $("#moradam-rank-position-number").text(0);
        $("#moradam-rank-keywords-count").text(0);
    }
    ...
    function update_shared_keywords(result) {
        ...
        $.each(top_keywords, function(key, value) {
            ...
            $('#moradam-competitors-keywords-count-top' + value).text(numberWithSpaces(current));
            $('#moradam-competitors-increase-count-top' + value).text(numberWithSpaces(avg));
            if (!$('#moradam-competitors-increase-count-top' + value).hasClass(top_kw_color)) {
                $('#moradam-competitors-increase-count-top' + value).removeClass().addClass(top_kw_color + ' bold');
            }
            if (!$('#moradam-competitors-arrow-style-top' + value).hasClass(arrow_style)) {
                $('#moradam-competitors-arrow-style-top' + value).removeClass().addClass(arrow_style);
            }
            if (!$('#moradam-competitors-arrow-color-top' + value).hasClass(arrow_color)) {
                $('#moradam-competitors-arrow-color-top' + value).removeClass().addClass(arrow_color);
            }
        });
        setTimeout(function() {
            $('.preloader-shared-keywords').css('display', 'none');
    ...
    function clear_shared_keywords() {
        let top_keywords = [1,3,10,30,50,100];
        $.each(top_keywords, function (key, value) {
            $('#moradam-competitors-keywords-count-top' + value).text(numberWithSpaces(0));
            $('#moradam-competitors-increase-count-top' + value).text(numberWithSpaces(0));
            if (!$('#moradam-competitors-increase-count-top' + value).hasClass('')) {
                $('#moradam-competitors-increase-count-top' + value).removeClass().addClass('bold');
            }
            if (!$('#moradam-competitors-arrow-style-top' + value).hasClass('arrow-default')) {
                $('#moradam-competitors-arrow-style-top' + value).removeClass().addClass('arrow-default');
            }
            if (!$('#moradam-competitors-arrow-color-top' + value).hasClass('arrow-default-color')) {
                $('#moradam-competitors-arrow-color-top' + value).removeClass().addClass('arrow-default-color');
            }
        });
        setTimeout(function() {
            $('.preloader-shared-keywords').css('display', 'none');
    ...
    function get_charts_data() {
        ...
        if (localStorage.getItem('moradam_competitors_selected_domains')) {
            let json_selected_domains = localStorage.getItem('moradam_competitors_selected_domains');
    ...
    function draw_chart (data) {
        ...
        if (localStorage.getItem('moradam_competitors_dynamics_top_filter')) {
            dynamics_top_counter = localStorage.getItem('moradam_competitors_dynamics_top_filter');
        }
                ...
                tooltip: {
                    ...
                    callbacks: {
                        ...
                        title: function(tooltipItem, data) {
                            let topN = 100;
                            if (localStorage.getItem('moradam_competitors_top_filter')) {
                                topN = localStorage.getItem('moradam_competitors_top_filter');
                            }
                            return 'Shared keywords on TOP ' + topN + ' on ' + moment(tooltipItem[0].label, 'YYYY-MM-DD').format('MMM Do, YYYY');
                        }
                    }
                },
            },
    ...
    function get_competitors_in_google_top() {
        ...
        if (localStorage.getItem('moradam_competitors_top_filter')) {
            request_data.top_filter = localStorage.getItem(('moradam_competitors_top_filter'));
        } else {
            request_data.top_filter = 100;
        }
        if (localStorage.getItem('moradam_competitors_filter_domain')) {
            request_data.site_filter_type = 'match';
            request_data.site_filter = localStorage.getItem('moradam_competitors_filter_domain');
        }
        remove_all_in_tbody();
        $('.no-data-found').remove();
            ...
            success: function(resp) {
                ...
                setTimeout(function() {
                    if (resp.result != null) {
                        ...
                        localStorage.setItem('moradam_competitors_selected_domains', json_selected_domains);
                        set_total_count_of_websites_in_table_head(resp.result.total);
                        insert_moradam_competitors_into_table(resp.result.result);
                        get_charts_data();
                        $('.preloader-competitors-table').css('display', 'none');
                    } else {
    ...
    function insert_moradam_competitors_into_table(websites, lastId) {
        $('.no-data-found').remove();
    ...
    $(window).scroll(function () {
        ...
        if($(window).scrollTop() == $(document).height() - $(window).height()) {
            ...
            if (allowScrollLoad == true) {
                ...
                if (localStorage.getItem('moradam_competitors_offset')) {
                    request_data.offset = localStorage.getItem('moradam_competitors_offset');
                } else {
                    localStorage.setItem('moradam_competitors_offset', defaultOffset);
                    request_data.offset = localStorage.getItem('moradam_competitors_offset');
                }
                if (localStorage.getItem('moradam_seo_selected_se')) {
                    request_data.se = localStorage.getItem('moradam_seo_selected_se');
                    ...
                    $.ajax({
                        ...
                        success: function(resp) {
                            if (resp.result != null && resp.result.total > 0) {
                                allowScrollLoad = true;
                                insert_moradam_competitors_into_table(resp.result.result, lastId);
                                if (resp.result.result.length == size) {
                                    let offset = localStorage.getItem('moradam_competitors_offset');
                                    let newOffset = parseInt(offset) + parseInt(size);
                                    localStorage.setItem('moradam_competitors_offset', newOffset );
                                }
                            }
                            $('.preloader-competitors-table').css('display', 'none');
                        },
